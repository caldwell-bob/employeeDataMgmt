...
function calcMonthsWorked(dateVar) {
  ...
  var OurDate = moment(dateVar,ourFormat);

  var duration = moment.duration(end.diff(dateVar, "months"));

  console.log();

  console.log("The months worked: " + duration);
}

$("#add-employee-btn").on("click", function(event) {
  event.preventDefault();
  // Get the input values
  var eName = $("#employee-name-input")
    .val()
    .trim();
  var role = $("#role-input")
    .val()
    .trim();
  var startDate = $("#start-input")
    .val()
    .trim();
  var rate = $("#rate-input")
    .val()
